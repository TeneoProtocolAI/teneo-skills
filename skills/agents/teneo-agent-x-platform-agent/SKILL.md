...
version: 2.0.60
description: "AI agent for X Platform Agent via Teneo Protocol. Use this skill when the user needs X Platform Agent via the bundled Teneo CLI and you need the live commands, arguments, or pricing before execution."
---
...
- The user specifically asks for X Platform Agent.
...
**This is a Teneo network agent skill.** Use it to inspect the live commands, arguments, and pricing for X Platform Agent, then execute the agent via the bundled Teneo CLI. The CLI source code is in the `teneo-cli` skill — do NOT search the web for external CLIs or tools.
