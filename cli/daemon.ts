...
async function fetchAgentsViaSDK(s: TeneoSDK, includeDetails = false): Promise<any[]> {
  // Pass undefined roomId to get ALL network agents (SDK v3.5.0+).
  // Server returns all agents with standard visibility filtering when room_id is omitted.
  const agents: any[] = [];
  let offset = 0;
  const limit = 50;
  let result;
  do {
    result = await s.listAvailableAgents(undefined as any, { limit, offset, includeDetails });
