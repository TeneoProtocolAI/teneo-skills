...
var WalletConfigurationError = class extends Error {
  constructor(message) {
    super(message);
    this.name = "WalletConfigurationError";
  }
};
function buildWalletRecoveryMessage(reason) {
...
function readMasterSecret(createIfMissing) {
  ensureWalletDir();
...
function normalizeWalletData(data) {
  ...
  for (const field of ["encryptedKey", "iv", "authTag", "createdAt"]) {
...
function saveWallet(data) {
  ensureWalletDir();
  nodeFs.writeFileSync(WALLET_FILE, JSON.stringify({ ...data, version: WALLET_VERSION }, null, 2), { mode: 384 });
  nodeFs.chmodSync(WALLET_FILE, 384);
}
function autoCreateWallet(options = {}) {
...
var SUPPORTED_WALLET_CHAINS = Object.keys(WALLET_CHAIN_MAP);
function formatSupportedWalletChains() {
  return SUPPORTED_WALLET_CHAINS.join(", ");
}
function formatSupportedWalletChainsForFlag() {
  return SUPPORTED_WALLET_CHAINS.join("|");
}
...
function parseUsdcAmount(amount) {
  ...
  const rawAmount = BigInt(wholePart) * 1000000n + BigInt(normalizedFraction);
...
async function stopDaemonBestEffort(port = getDaemonPort()) {
  if (port) {
    try {
      await fetch(`http://localhost:${port}/stop`, { method: "POST", signal: AbortSignal.timeout(2e3) });
      await sleep(200);
    } catch {
    }
  }
...
program.name("teneo-cli").version("2.0.48").description("Teneo Protocol CLI. Private keys are NEVER transmitted.").option("--json", "Machine-readable JSON output");
...
program.command("quote").description("Check price for a command (does not execute)").argument("<message>").option("--room <roomId>").option("--chain <chain>", `Payment chain (${formatSupportedWalletChainsForFlag()})`).option("--network <network>", "Payment network (alias for --chain)").action(async (message, opts) => {
  const room = await resolveRoom(opts.room);
  const chain = opts.chain || opts.network;
...
program.command("wallet-balance").description("Check USDC and native token balances on supported chains").option("--chain <chain>", `Specific chain (${formatSupportedWalletChainsForFlag()})`).action(async (opts) => {
...
program.command("wallet-send").description("Send USDC to any address").argument("<amount>", "Amount in USDC").argument("<to>", "Destination address").argument("<chain>", `Chain (${formatSupportedWalletChainsForFlag()})`).action(async (amountStr, to, chainName) => {
...
function saveAgentRegistryEntry(agentId, updates) {
  ensureWalletDir();
...
function saveTokenToRegistry(agentId, tokenId, workDir) {
  saveAgentRegistryEntry(agentId, {
    token_id: tokenId,
    ...workDir ? { work_dir: workDir } : {}
  });
}
function saveWorkDirToRegistry(agentId, workDir) {
...
Example:
  ...
  if (!metadataOnly) {
    const result = await scaffoldAgent(metadata, {
      ...
      useCliKey: opts.useCliKey ? true : void 0,
      newKey: !!opts.newKey
    });
    const metaFile = `${result.dir}/${agentId}-metadata.json`;
    const envLabel = result.keyMode === "cli_wallet" ? "CLI wallet key (default)" : "separate private key (--new-key)";
    if (JSON_FLAG) {
      out({
        status: "created",
        agent_id: agentId,
...
function getLocalServiceWorkDir(agentId) {
  const platform2 = nodeOs.platform();
  try {
    if (platform2 === "darwin") {
...
function loadPrivateKeyFromEnvFile(workDir) {
  try {
