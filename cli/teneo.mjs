...
async function startDaemon() {
  ...
  const child = usePrecompiled ? spawn2("node", [mjsPath], { detached: true, stdio: "ignore", cwd: __dirname, env: { ...process.env } }) : spawn2("npx", ["tsx", tsPath], { detached: true, stdio: "ignore", cwd: __dirname, env: { ...process.env } });
  child.unref();
  for (let i = 0; i < 30; i++) {
    await sleep(500);
...
program.name("teneo-cli").version("2.0.26").description("Teneo Protocol CLI. Private keys are NEVER transmitted.").option("--json", "Machine-readable JSON output");
...
agentCmd.command("init").description("Create agent metadata JSON and scaffold Go project").option("--name <name>", "Agent name").option("--id <id>", "Agent ID (kebab-case)").option("--type <type>", "Agent type (command|nlp|commandless|mcp)").option("--template <template>", "Go template: enhanced (default) or simple-openai", "enhanced").option("--description <desc>", "Agent description").option("--short-description <desc>", "Short description").option("--category <cat>", "Category (can specify multiple)", (val, prev) => prev.concat(val), []).option("--no-scaffold", "Only create metadata JSON, skip Go project scaffolding").option("--use-cli-key", "Reuse the CLI wallet key for the agent").action(async (opts) => {
  let name = opts.name;
  let agentId = opts.id;
  ...
  if (!name || !description || !shortDescription || categories.length === 0) {
    try {
      const { input, select, checkbox } = await Promise.resolve().then(() => (init_esm15(), esm_exports));
...
agentCmd.command("withdraw").description("Withdraw agent from public back to private").argument("<agentId>", "Agent ID").argument("<tokenId>", "NFT token ID").action(async (agentId, tokenIdStr) => {
  const tokenId = parseInt(tokenIdStr);
  if (isNaN(tokenId)) fail("tokenId must be a number");
  const key = requireKey();
  const hex = key.startsWith("0x") ? key : `0x${key}`;
...
agentCmd.command("status").description("Check deployment status of an owned agent").argument("<agentId>", "Agent ID").action(async (agentId) => {
...
agentCmd.command("scaffold").description("Scaffold a Go agent project from existing metadata JSON").argument("<metadataFile>", "Path to metadata JSON file").option("--template <template>", "Go template: enhanced (default) or simple-openai", "enhanced").option("--use-cli-key", "Reuse the CLI wallet key for the agent").action(async (metadataFile, opts) => {
  if (!nodeFs.existsSync(metadataFile)) fail(`File not found: ${metadataFile}`);
  let meta;
  try {
    meta = JSON.parse(nodeFs.readFileSync(metadataFile, "utf8"));
  } catch {
    fail(`Invalid JSON in ${metadataFile}`);
  }
  const errors = validateMetadata(meta);
  if (errors.length > 0) {
...
agentCmd.command("install").description("Install agent as a background service (auto-restarts on crash/reboot)").argument("<directory>", "Path to the agent project directory").action(async (directory) => {
  const { execSync } = await import("node:child_process");
  const platform2 = nodeOs.platform();
...
agentCmd.command("uninstall").description("Stop and remove agent background service").argument("<agentId>", "Agent ID").action(async (agentId) => {
  const { execSync } = await import("node:child_process");
  const platform2 = nodeOs.platform();
  if (platform2 === "darwin") {
    const plistPath = getMacPlistPath(agentId);
...
agentCmd.command("service-status").description("Check if an agent is running as a background service").argument("<agentId>", "Agent ID").action(async (agentId) => {
  const { execSync } = await import("node:child_process");
  const platform2 = nodeOs.platform();
  if (platform2 === "darwin") {
    const plistPath = getMacPlistPath(agentId);
